...
export function ProjectionChart({ paths }: { paths: PathResult[] }) {
  ...
  return (
    <div className="life-chart-wrap">
      <svg className="life-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Projected spendable assets under three deterministic paths">
        ...
        {series[0]?.points.map((row, index) => index % 5 === 0 ? (
          <text key={row.month} x={point(0, index).split(",")[0]} y={height - 5} textAnchor="middle">{Math.floor(row.age_months / 12)}</text>
        ) : null)}
      </svg>
      <div className="life-chart-legend">
