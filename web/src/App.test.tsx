...
describe("application states", () => {
  ...
  it("keeps the exact visible navigation order and exposes the conditional Review count", async () => {
    ...
    expect(Array.from(navigation.querySelectorAll("button")).map((button) => button.getAttribute("aria-label")?.replace(/, \d+ issues$/u, ""))).toEqual([
      "Cash Flow", "Goals", "Activity", "Overview", "Accounts", "Income", "Wealth", "Housing Move", "Retirement", "Lab", "Add account", "Review",
    ]);
    ...
    expect(within(screen.getByRole("group", { name: "Planning" })).getAllByRole("button").map((button) => button.getAttribute("aria-label"))).toEqual(["Housing Move", "Retirement", "Lab"]);
  ...
  it("reveals an active mobile item and keeps every grouped route reachable", async () => {
    ...
    render(<App />);
    await screen.findByRole("heading", { name: "Cash Flow" });
    const expectedRoutes = ["Cash Flow", "Goals", "Activity", "Overview", "Accounts", "Income", "Wealth", "Housing Move", "Retirement", "Lab", "Add account", "Review, 1 issues"];
