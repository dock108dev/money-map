...
it("holds the draft steady while a save is pending and re-enables editing on failure", async () => {
  ...
  expect(screen.getByText("Copy as alternative")).toBeDisabled();
  expect(screen.getByText("Discard unsaved changes")).toBeDisabled();
  finishSave(new Response(JSON.stringify({ detail: "Save failed" }), { status: 409 }));
