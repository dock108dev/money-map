...
Status: implementation proof only. Live owner rehearsal, owner data, owner decisions, the
bounded installed smoke and final acceptance require a qualified candidate and separate owner execution. The former
Campaigns A-J matrix is optional dedicated-runner soak coverage, not a cutover prerequisite.

## Reused authority

Cutover readiness extends the shared `DataHomeManager`; it is not a second migration system. The
...
and cleanup are covered by the data-home authority and cutover regression checks.
...
1. Freeze the exact owner-local candidate and complete the bounded headless and installed smoke.
...
Implementation evidence is synthetic, sanitized, ignored, and non-promotable. It may record safe counts,
classifications, test results, schema, and source commit; it may not contain private paths, raw
identities, data values, credentials, sessions, provider responses, or owner answers. Fresh
worksheets generated by `owner_worksheet()` keep every owner-controlled response blank;
`validate_owner_worksheet()` rejects prepopulation before a live run. The checked-in
`owner-cutover-worksheet.json` is the ongoing owner record and may contain explicitly supplied
responses and identity-bound engineering evidence from completed sessions. Recorded responses
must not be inferred or copied into a fresh worksheet.
...
release approval. Those remain in the owner-local delivery plan.
